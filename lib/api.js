...
export async function getAllBolgPosts() {

  try {
    const response = await fetch(
      `${WP_API_URL}posts?_embed&categories=4`,
      {
        next: { revalidate: 10 },
        headers: {
          'Accept': 'application/json',
...
export async function getAllServicePosts(id) {

  try {
    const response = await fetch(
      `${WP_API_URL}posts?_embed&categories=${id}`,
      {
        next: { revalidate: 10 },
        headers: {
          'Accept': 'application/json',
